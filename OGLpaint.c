...
void draw_circle(GLfloat p, GLfloat q, GLfloat r)    /* to draw a CIRCLE using MIDPOINT CIRCLE DRAWING algorithm */
{
	...
	plotpixels(p, q, x, y);
	Circulo nuevo = new Circulo(x,y,)
	listaCirculos.push_back(nuevo);
}
...
/*
		...
		glColor3f(0, 1, 1);
		drawstring(22*wh/45, 42*wh/45 , 0.0, "A Project On");
		...
		drawstring(8*wh/45, 29*wh/45, 0.0, "Nitya.B");
		drawstring(8*wh/45, 27*wh/45, 0.0, "USN:1CE09CS034");


		drawstring(35*wh/45, 29*wh/45, 0.0, "Divya.P");
		drawstring(35*wh/45, 27*wh/45, 0.0, "USN:1CE09CS018");

		drawstring(5*wh/45, 15*wh/45, 0.0, "Department of Computer Science");
		drawstring(5*wh/45, 13*wh/45, 0.0, "City Engineering College");
*/
     	wel=1;
